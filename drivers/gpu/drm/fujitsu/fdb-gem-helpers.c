...
_drm_gem_create_mmap_offset_size(struct drm_gem_object *obj, size_t size)
{
	...
	/* pr_err("_drm_gem_create_mmap_offset_size drm_gem_obj=%p\n", obj); */
...
#if 0
	pr_err("inserting ht %p key 0x%lx\n",
			&dev->map_hash /*&mm->offset_hash*/,
						list->file_offset_node->start);
#endif
	list->hash.key = list->file_offset_node->start;
	ret = drm_ht_insert_item(
			/*&dev->map_hash */&mm->offset_hash, &list->hash);
	if (ret) {
