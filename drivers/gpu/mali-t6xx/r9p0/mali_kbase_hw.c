...
void kbase_hw_set_features_mask(struct kbase_device *kbdev)
{
	...
	dev_err(kbdev->dev, "mali_kbase_hw.c : kbase_hw_set_features_mask() product_id=0x%x",product_id);
		switch (product_id) {
		case GPU_ID_PI_TFRX:
			/* FALLTHROUGH */
		case GPU_ID_PI_T86X:
...
int kbase_hw_set_issues_mask(struct kbase_device *kbdev)
{
	...
	if (impl_tech != IMPLEMENTATION_MODEL) {
			dev_err(kbdev->dev, "mali_kbase_hw.c : kbase_hw_set_issues_mask() HARD impl_tech=0x%x gpu_id=0x%x ",impl_tech, gpu_id);
			switch (gpu_id) {
		...
		/* Software model */
			dev_err(kbdev->dev, "mali_kbase_hw.c : kbase_hw_set_issues_mask() SOFT impl_tech=0x%x product_id=0x%x ",impl_tech, product_id);
			switch (product_id) {
			case GPU_ID_PI_T60X:
