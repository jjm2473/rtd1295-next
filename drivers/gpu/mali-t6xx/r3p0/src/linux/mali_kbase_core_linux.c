...
#include "../common/mali_kbase.h"
#include "../common/mali_kbase_uku.h"
#include "../common/mali_midg_regmap.h"
#include "../linux/mali_kbase_mem_linux.h"
#include "../linux/mali_kbase_config_linux.h"
...
#include "../common/mali_kbase_8401_workaround.h"
#include "../common/mali_kbase_hw.h"
#ifdef CONFIG_SYNC
...
//pr_err("mali0: JOB IRQ\n");
	...
	if (!kbdev->pm.driver_ready_for_irqs)
		dev_err(kbdev->osdev.dev, "%s: JOB irq %d irqstatus 0x%x before driver is ready\n",
				__func__, irq, val );
	...
	dev_dbg(kbdev->osdev.dev, "%s: JOB irq %d irqstatus 0x%x\n", __func__, irq, val);
smp_wmb();
...
//pr_err("mali0: MMU IRQ\n");
	spin_lock_irqsave(&kbdev->pm.gpu_powered_lock, flags);
	val = kbase_reg_read(kbdev, MMU_REG(MMU_IRQ_STATUS), NULL);

	if (!kbdev->pm.gpu_powered) {
		pr_err("mali0: MMU IRQ ignored\n");
		/* GPU is turned off - IRQ is not for us */
		spin_unlock_irqrestore(&kbdev->pm.gpu_powered_lock, flags);
		return IRQ_HANDLED;
	}
...
#ifdef CONFIG_MALI_DEBUG
	if (!kbdev->pm.driver_ready_for_irqs)
		dev_err(kbdev->osdev.dev, "%s: MMU irq %d irqstatus 0x%x before driver is ready\n",
				__func__, irq, val );
#endif /* CONFIG_MALI_DEBUG */
	spin_unlock_irqrestore(&kbdev->pm.gpu_powered_lock, flags);
	...
	//dev_err(kbdev->osdev.dev, "%s: MMU irq %d irqstatus 0x%x\n", __func__, irq, val);
...
//pr_err("mali0: GPU IRQ=======================================================================================================\n");

	spin_lock_irqsave(&kbdev->pm.gpu_powered_lock, flags);
	if (!kbdev->pm.gpu_powered) {
		pr_err("mali0: ignoring GPU IRQ\n");
		/* GPU is turned off - IRQ is not for us */
		spin_unlock_irqrestore(&kbdev->pm.gpu_powered_lock, flags);
		return IRQ_HANDLED;
	}
...
#ifdef CONFIG_MALI_DEBUG
	if (!kbdev->pm.driver_ready_for_irqs)
		dev_info(kbdev->osdev.dev, "%s: GPU irq %d irqstatus 0x%x before driver is ready\n",
				__func__, irq, val );
#endif /* CONFIG_MALI_DEBUG */
	spin_unlock_irqrestore(&kbdev->pm.gpu_powered_lock, flags);
...
//mdelay(2);
//	dev_err(kbdev->osdev.dev, "%s: GPU %d irqstatus 0x%x\n", __func__, irq, val);
smp_wmb();
...
static mali_error kbasep_common_test_interrupt_handlers(kbase_device * const kbdev)
{
	...
	dev_info(osdev->dev, "Interrupts are correctly assigned.\n");
...
static int kbase_install_interrupts(kbase_device *kbdev)
{
	...
	int err;
	u32 i;
	static const char *irq_name[] = { "JOB", "MMU", "GPU" };
	static char mali_irq_names[3][64];
	...
	kbase_reg_write(kbdev, GPU_CONTROL_REG(GPU_IRQ_CLEAR), 0xffffffff, NULL);
#endif
	BUG_ON(nr > PLATFORM_CONFIG_IRQ_RES_COUNT);	/* Only 3 interrupts! */

	for (i = 0; i < nr; i++) {
		sprintf(mali_irq_names[i], "%s-%s", dev_name(osdev->dev), irq_name[i]);
		err = request_irq(osdev->irqs[i].irq, kbase_handler_table[i],
				osdev->irqs[i].flags,
				mali_irq_names[i], kbase_tag(kbdev, i));
		if (err) {
			dev_err(osdev->dev, "Can't request interrupt %d (index %d)\n", osdev->irqs[i].irq, i);
#ifdef CONFIG_SPARSE_IRQ
			dev_err(osdev->dev, "You have CONFIG_SPARSE_IRQ support enabled - is the interrupt number correct for this configuration?\n");
...
static int kbase_platform_power_clock_init(kbase_device *kbdev)
{
	...
	/* Turn on G3D clock */
	kbdev->clocks[0] = clk_get(dev, "g3d");
	if (IS_ERR(kbdev->clocks[0])) {
		kbdev->clocks[0] = NULL;
		printk(KERN_ERR "failed to clk_get [clk_g3d]\n");
		goto out;
	}
	clk_prepare_enable(kbdev->clocks[0]);
	clk_set_rate(kbdev->clocks[0], 800 * 1000 * 1000);

	kbdev->clocks[1] = clk_get(dev, "mout_gpll");
	if (IS_ERR(kbdev->clocks[1])) {
		printk(KERN_ERR "failed to clk_get [mout_gpll]\n");
		goto out;
	}
	clk_prepare_enable(kbdev->clocks[1]);

	kbdev->clocks[2] = clk_get(dev, "aclk_g3d");
	if (IS_ERR(kbdev->clocks[2])) {
		printk(KERN_ERR "failed to clk_get [sclk_g3d]\n");
		goto out;
	}
	clk_prepare_enable(kbdev->clocks[2]);

	kbdev->clocks[3] = clk_get(dev, "fout_gpll");
	if (IS_ERR(kbdev->clocks[3])) {
		printk(KERN_ERR "failed to clk_get [fout_gpll]\n");
		goto out;
	}
	clk_prepare_enable(kbdev->clocks[3]);
...
static void kbase_platform_power_clock_destroy(struct kbase_device *kbdev)
{
	int n;

	for (n = 0; n < ARRAY_SIZE(kbdev->clocks); n++)
		if (kbdev->clocks[n]) {
			clk_disable_unprepare(kbdev->clocks[n]);
			clk_put(kbdev->clocks[n]);
			kbdev->clocks[n] = NULL;
		}
}
...
static int kbase_platform_device_probe(struct platform_device *pdev)
{
	...
	const kbase_attribute *platform_data;
	int err;
	int i;
	...
	osdev = &kbdev->osdev;
	osdev->dev = &pdev->dev;
	platform_data = kbasep_get_platform_config()->attributes;
	err = platform_device_add_data(pdev, platform_data,
		kbasep_get_config_attribute_count(platform_data) *
			  sizeof(kbasep_get_platform_config()->attributes[0]));
	if (err) {
	...
	if (err) {
		dev_err(osdev->dev, "Failed clock ini\n");
		goto out_term_dev;
	}
...
static int kbase_common_device_remove(struct kbase_device *kbdev)
{
	...
	kbase_platform_power_clock_destroy(kbdev);
...
static const struct of_device_id f_mb86s70_malit6xx_dt_ids[] = {
        { .compatible = "fujitsu,malit6xx" },
            { /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, f_mb86s70_malit6xx_dt_ids);
...
static struct platform_driver kbase_platform_driver = {
	...
	.driver = {
		   ...
		   .owner = THIS_MODULE,
		   .pm = &kbase_pm_ops,
                   .of_match_table = f_mb86s70_malit6xx_dt_ids,
		   },
};
