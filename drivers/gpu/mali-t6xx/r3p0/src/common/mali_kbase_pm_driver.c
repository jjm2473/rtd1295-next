...
//	dev_err(kbdev->osdev.dev, "%s: type:%d, powerup 0x%llX\n", __func__, type, powerup);
	kbase_pm_invoke(kbdev, type, powerup, ACTION_PWRON);
//	dev_err(kbdev->osdev.dev, "%s: type:%d, powerdown 0x%llX\n", __func__, type, powerdown);
...
void kbase_pm_clock_on(kbase_device *kbdev)
{
	...
	pr_info("mali: kbase_pm_clock_on\n");
...
void kbase_pm_clock_off(kbase_device *kbdev)
{
	...
	pr_info("mali: kbase_pm_clock_off\n");
...
pr_err("mali: kbase_pm_init_hw soft reset failed\n");
...
pr_err("mali: reset attempt failed\n");
