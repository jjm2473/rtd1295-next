...
static int pmu_nr_irqs;
static int pmu_irqs[CCI400_PMU_MAX_HW_EVENTS];
static unsigned long pmu_active_irqs;
...
static int cci_pmu_request_irq(struct arm_pmu *cci_pmu, irq_handler_t handler)
{
	...
	*/
	for (i = 0; i < pmu_nr_irqs; i++) {
		err = request_irq(pmu_irqs[i], handler, IRQF_SHARED, "arm-cci-pmu", cci_pmu);
		if (err) {
...
static void cci_pmu_free_irq(struct arm_pmu *cci_pmu)
{
	...
	for (i = 0; i < pmu_nr_irqs; i++) {
		if (!test_and_clear_bit(i, &pmu_active_irqs))
			continue;

		free_irq(pmu_irqs[i], cci_pmu);
	}
}
...
static int cci_pmu_probe(struct platform_device *pdev)
{
	 ...
	 */
	pmu_nr_irqs = 0;
	for (i = 0; i < CCI400_PMU_MAX_HW_EVENTS; i++) {
		irq = platform_get_irq(pdev, i);
		if (irq < 0)
			break;

		if (is_duplicate_irq(irq, pmu_irqs, pmu_nr_irqs))
			continue;

		pmu_irqs[pmu_nr_irqs++] = irq;
	}
	 ...
	 * of counters.
	 */
	if (i < CCI400_PMU_MAX_HW_EVENTS) {
		dev_warn(&pdev->dev, "In-correct number of interrupts: %d, should be %d\n",
			i, CCI400_PMU_MAX_HW_EVENTS);
		return -EINVAL;
	}
