...
extern int reject_32bpp;

struct bus_type f_fdb_bus_type;
					...
					struct fb_var_screeninfo *var)
{
	...
	var->pixclock = (u64)1000000000 / dt->pixelclock.typ;
	var->sync = 0;
					...
					struct fdb_video_timings *fvt)
{
	...
	dt->pixelclock.typ = (u64)1000000000 / var->pixclock;
	dt->flags = 0;
							...
							struct f_fdb_ops *ops)
{
	...
	/* is he a head that can simple-bind to an fb? */
	node = of_parse_phandle(dev->of_node, "simple-bind", 0);
	if (node) {
		dev_info(dev, "seen simple-bind\n");
		if (of_property_read_u32(node, "simple", &n)) {
			dev_info(dev, "not simple fb\n");
			return 0;
		}
		if (!n) {
			dev_info(dev, "fb in drm mode\n");
			return 0;
		}
		dev_info(dev, "fb in simple mode\n");
		fdb_name_to_child(dev, of_node_full_name(node), &child1);
		if (!child1) {
			dev_err(dev, "Unable to find bind peer\n");
			return 0;
		}
		dev_info(dev, "binding us to simple fb %s\n", of_node_full_name(node));
		/* tag the crtc as reporting to us */
		child->ops->bind_to_source(child, child1);
		
		/* enable it */
		child->ops->enable(child);
		
		/* get timings from fb */
		child1->ops->get_timings(child1, &timings);

		/* apply timings to head */
...
int fdb_name_to_child(struct device *dev,
				const char *name, struct f_fdb_child **child)
{
	struct f_fdb *bus = f_fdb_from_dev(dev);
	int n;
	...
	for (n = 0; n < bus->count_fdb_children; n++) {
		...
		//pr_err("fdb_name_to_child: %s %s\n",
		//	of_node_full_name(bus->child[n]->dev->of_node), name);

		if (!strcmp(of_node_full_name(bus->child[n]->dev->of_node),
				name)) {
			*child = bus->child[n];
			//pr_info("fdb_name_to_child: %s -> %p\n", name, *child);
...
int fdb_callback_each(struct device *dev,
	       int (*callback)(struct f_fdb_child *child, void *arg), void *arg)
{
	struct f_fdb *bus = f_fdb_from_dev(dev);
	int n;
...
static int f_fdb_bus_match(struct device *dev, struct device_driver *driver)
{
...
f_fdb_probe(struct platform_device *pdev)
{
	...
	f_fdb_bus_type.name = priv->name;
	f_fdb_bus_type.match = f_fdb_bus_match;
	f_fdb_bus_type.dev_attrs = default_dev_attrs;
	//f_fdb_bus_type.drv_attrs = default_drv_attrs;

	ret = bus_register(&f_fdb_bus_type);
	if (ret) {
...
static int f_fdb_remove(struct platform_device *pdev)
{
	struct f_fdb *priv = platform_get_drvdata(pdev);

	bus_unregister(&f_fdb_bus_type);
	kfree(priv);
