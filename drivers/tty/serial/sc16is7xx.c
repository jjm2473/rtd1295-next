...
struct sc16is7xx_one {
	...
	void					(*rx_cb_handler)(const void *data, size_t len, void *arg);
	void					*cb_arg;
};
...
/* Most of this code is borrowed from uart_write(). */
static int sc16is7xx_put_circ_buf(
		struct circ_buf *circ, spinlock_t *lock, const unsigned char *buf, int count, int buf_sz)
{
	unsigned long flags;
	int c, ret = 0;
	...
	while (1) {
		c = CIRC_SPACE_TO_END(circ->head, circ->tail, buf_sz);
		if (count < c)
			c = count;
		if (c <= 0)
			break;
		memcpy(circ->buf + circ->head, buf, c);
		circ->head = (circ->head + c) & (buf_sz - 1);
		buf += c;
		count -= c;
		ret += c;
	}
				...
				unsigned int iir)
{
	...
	while (rxlen) {
		...
		if (one->not_tty) {
			...
			/*
			 * We do not report error status to upper layer.
			 * Upper layer guarantees to detect buffer overflow and link error condition.
			 * (use CRC, checksum, etc)
			 */
			one->rx_cb_handler(s->buf, (size_t)bytes_read, one->cb_arg);

			if (unlikely(lsr)) {
				dev_warn_ratelimited(port->dev,
				     "FIFO data error(%#x) is detected. (line:%d)\n", lsr, port->line);
			}
...
static void sc16is7xx_handle_tx(struct uart_port *port)
{
	...
	if (!one->not_tty && unlikely(port->x_char)) {
	...
	if (uart_circ_empty(xmit) || (!one->not_tty && uart_tx_stopped(port)))
		return;
	...
	if (!one->not_tty && uart_circ_chars_pending(xmit) < WAKEUP_CHARS)
...
static void sc16is7xx_port_irq(struct sc16is7xx_port *s, int portno)
{
	...
	} while (unexpected < 30); /* DEFAULT_RATELIMIT_BURST * 3 */
}
...
static bool sc16is7xx_chk_not_tty(u32 port, const u32 *port_array)
{
	bool ret = false;
	int i;
			   ...
			   struct regmap *regmap, int irq, unsigned long flags)
{
	...
	int i, ret, not_tty_cnt;
	struct sc16is7xx_port *s;
	u32 fixed_clk, not_tty[SC16IS7XX_MAX_DEVS];

	if (IS_ERR(regmap))
		return PTR_ERR(regmap);

	memset(not_tty, 0xff, sizeof(not_tty));
	for (i = 0; i < SC16IS7XX_MAX_DEVS; i++) {
		if (of_property_read_u32_index(dev->of_node, "not-tty", i, &not_tty[i])) {
			break;
		}
	...
	if (!of_property_read_u32(dev->of_node, "fixed-clk-freq", &fixed_clk)) {
		s->clk = ERR_PTR(-EINVAL);
		freq = (unsigned long)fixed_clk;
	}
	else {
	...
	for (i = 0; i < devtype->nr_uart; ++i) {
		...
		if (sc16is7xx_chk_not_tty(i, not_tty)) {
			struct circ_buf *cbuf = &s->p[i].tx_buf;

			cbuf->buf = devm_kzalloc(dev, UART_XMIT_SIZE, GFP_KERNEL);
			if (!cbuf->buf) {
				ret = -ENOMEM;
				goto out_ports;
			}
			...
			dev_info(dev, "uart_add_one_port result:%d\n", ret);
		}
...
{
	struct sc16is7xx_port *s = dev_get_drvdata(&client->dev);
	struct sc16is7xx_one *one;
	int ret;

	if (port < 0 || port >= s->devtype->nr_uart || !buf || count <= 0) {
		ret = -EINVAL;
		goto end;
	}

	one = to_sc16is7xx_one(&s->p[port].port, port);
	if (!one->not_tty) {
		ret = -EINVAL;
		goto end;
	}

	ret = sc16is7xx_put_circ_buf(&one->tx_buf, &s->p[port].port.lock, buf, count, UART_XMIT_SIZE);
	if (ret > 0)
		sc16is7xx_start_tx(&s->p[port].port);

end:
	return ret;
}
EXPORT_SYMBOL(sc16is7xx_i2c_send_async);
...
static int sc16is7xx_i2c_pm_suspend(struct device *dev)
{
...
static int sc16is7xx_i2c_pm_resume(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
...
static const struct dev_pm_ops sc16is7xx_i2c_pm_ops = {
       SET_SYSTEM_SLEEP_PM_OPS(sc16is7xx_i2c_pm_suspend, sc16is7xx_i2c_pm_resume)
};
...
{
	struct sc16is7xx_port *s = dev_get_drvdata(sc16_dev);
	struct sc16is7xx_one *one;
	int ret;

	if (port < 0 || port >= s->devtype->nr_uart || !termios || !cb_handler) {
		ret = -EINVAL;
		goto end;
	}
	...
	one->rx_cb_handler = cb_handler;
	one->cb_arg = cb_arg;

	sc16is7xx_set_termios(&s->p[port].port, termios, NULL);

	ret = sc16is7xx_startup(&s->p[port].port);

end:
	return ret;
}
EXPORT_SYMBOL(sc16is7xx_startup_not_tty);
...
void sc16is7xx_shutdown_not_tty(struct device *sc16_dev, int port)
{
	...
	if (port < 0 || port >= s->devtype->nr_uart) {
		return;
	}

	one = to_sc16is7xx_one(&s->p[port].port, port);
	if (one->not_tty) {
		sc16is7xx_shutdown(&s->p[port].port);
	}
}
EXPORT_SYMBOL(sc16is7xx_shutdown_not_tty);
