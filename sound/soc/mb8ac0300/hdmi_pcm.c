/*
 * linux/sound/soc/mb8ac0300/hdmi_pcm.c

 * copy from linux/sound/soc/mb8ac0300/mb8ac0300_pcm.c
...
static int hdmac_hdmi_i2s0;
static int hdmac_hdmi_i2s1;
static int hdmac_hdmi_i2s2;
static int hdmac_hdmi_i2s3;

#define DRIVER_NAME "hdmi_pcm"
...
static int hdmi_pcm_enqueue(struct snd_pcm_substream *substream);

static int hdmi_pcm_preallocate_dma_buffer(struct snd_pcm *pcm,
						int stream)
{
...
/**
 * hdmi_pcm_buffdone - DMA callback function
 * @channel:	dma channel
 * @irq_data:	pointer of request information saved by hdmac_enqueue
 * @state:	result of dma transfer
 */
static void hdmi_pcm_buffdone(u32 channel, void *irq_data, int state)
{
...
/**
 * hdmi_pcm_enqueue - add dma transfer request to queue
...
static int hdmi_pcm_enqueue(struct snd_pcm_substream *substream)
{
	...
	hdmac_req_info->req_data.size = len;
		...
		pr_debug("*** hdmi_pcm_enqueue: playback type\n");
	...
	hdmac_req_info->req_data.callback_fn = hdmi_pcm_buffdone;
...
/**
 * hdmi_pcm_hw_params - Set parameters of sound device
 * @substream:	the pcm substream
 * @params:	pointer of hardware parameters
 *
 * Returns 0 if no error, -EINVAL or other negative errno on failure
 */
static int hdmi_pcm_hw_params(struct snd_pcm_substream *substream,
	struct snd_pcm_hw_params *params)
{
	...
	prtd->params.dmaca_is = hdmac_hdmi_i2s0;
...
/**
 * hdmi_pcm_hw_free - free hardware used by pcm
 * @substream:	the pcm substream
 *
 * Returns 0
 */
static int hdmi_pcm_hw_free(struct snd_pcm_substream *substream)
{
	struct mb8ac0300_pcm_runtime_data *prtd =
			substream->runtime->private_data;
...
/**
 * hdmi_pcm_prepare - prepare process of pcm
 * @substream:	the pcm substream
 *
 * Returns 0 if no error, negative errno on failure
 */
static int hdmi_pcm_prepare(struct snd_pcm_substream *substream)
{
	struct mb8ac0300_pcm_runtime_data *prtd =
			substream->runtime->private_data;
	int ret = 0;
	...
	ret = hdmi_pcm_enqueue(substream);

	return ret;
}

/**
 * hdmi_pcm_trigger - start/stop dma by judging command
 * @substream:	the pcm substream
 * @cmd:	pcm trigger command
 *
 * Returns 0 if no error, -EINVAL or other negative errno on failure
 */
static int hdmi_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
	struct mb8ac0300_pcm_runtime_data *prtd =
			substream->runtime->private_data;
	int ret = 0;
...
/**
 * hdmi_pcm_pointer - get the number of frame waiting for sending
 * @substream:	the pcm substream
 *
 * Returns the number of frame waiting for sending
 */
static snd_pcm_uframes_t
hdmi_pcm_pointer(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	struct mb8ac0300_pcm_runtime_data *prtd = runtime->private_data;
	unsigned long res;
	...
	if (hdmac_getposition(prtd->params.channel, (u32 *)&src, (u32 *)&dst) == 0) {
		pr_debug("Pointer %llx %llx\n", (u64)src, (u64)dst);
...
/**
 * hdmi_pcm_open - pcm open process
 * @substream:	the pcm substream
 *
 * Returns 0 if no error, -ENOMEM or other negative errno on failure
 */
...
/**
 * hdmi_pcm_close - pcm close process
 * @substream:	the pcm substream
 *
 * Returns 0
 */
static int hdmi_pcm_close(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	struct mb8ac0300_pcm_runtime_data *prtd = runtime->private_data;
...
/**
 * hdmi_pcm_mmap - mapping dma buffer virtual address and physical address
 * @substream:	the pcm substream
 * @vma:	pointer of virtual area
 *
 * Returns 0 if no error, negative errno on failure
 */
static int hdmi_pcm_mmap(struct snd_pcm_substream *substream,
...
static struct snd_pcm_ops hdmi_pcm_ops = {
	...
	.hw_free	= hdmi_pcm_hw_free,
	.prepare	= hdmi_pcm_prepare,
	.trigger	= hdmi_pcm_trigger,
	.pointer	= hdmi_pcm_pointer,
	.mmap		= hdmi_pcm_mmap,
};

/**
 * hdmi_pcm_free - free playback/capture dma buffer
 * @pcm:	pointer of pcm
 */
static void hdmi_pcm_free(struct snd_pcm *pcm)
{
	struct snd_pcm_substream *substream;
...
static int hdmi_pcm_new(struct snd_soc_pcm_runtime *rtd)
{
	...
	if (pcm->streams[SNDRV_PCM_STREAM_PLAYBACK].substream) {
		...
		ret = hdmi_pcm_preallocate_dma_buffer(pcm,
...
static struct snd_soc_platform_driver hdmi_soc_platform = {
	.ops		= &hdmi_pcm_ops,
	.pcm_new	= hdmi_pcm_new,
	.pcm_free	= hdmi_pcm_free,
};

/**
 * hdmi_pcm_probe - pcm module probe function
 * @pdev:	pointer of platform device
 *
 * Returns 0 if no error, -EINVAL errno on failure
 */
static int hdmi_pcm_probe(struct platform_device *pdev)
{
	const int *p;
	...
	if (pdev->dev.of_node) {
		...
		p = of_get_property(pdev->dev.of_node, "hdmac-hdmi-i2s0", NULL);
		if (p)
			hdmac_hdmi_i2s0 = be32_to_cpu(*p);
		p = of_get_property(pdev->dev.of_node, "hdmac-hdmi-i2s1", NULL);
		if (p)
			hdmac_hdmi_i2s1 = be32_to_cpu(*p);
		p = of_get_property(pdev->dev.of_node, "hdmac-hdmi-i2s2", NULL);
		if (p)
			hdmac_hdmi_i2s2 = be32_to_cpu(*p);
		p = of_get_property(pdev->dev.of_node, "hdmac-hdmi-i2s3", NULL);
		if (p)
			hdmac_hdmi_i2s3 = be32_to_cpu(*p);
	}
	...
	dev_info(&pdev->dev, "Using HDMAC %d trigger inputs i2s0=0x%x, i2s1=0x%x, i2s2=0x%x, i2s3=0x%x\n",
			hdmac_index, hdmac_hdmi_i2s0, hdmac_hdmi_i2s1,
			hdmac_hdmi_i2s2, hdmac_hdmi_i2s3);

	return snd_soc_register_platform(&pdev->dev, &hdmi_soc_platform);
}

/**
 * hdmi_pcm_remove - pcm module remove function
 * @pdev:	pointer of platform device
 *
 * Returns 0
 */
static int hdmi_pcm_remove(struct platform_device *pdev)
{
	pr_debug("Entered %s\n", __func__);
...
{
	return &hdmi_soc_platform;
}
EXPORT_SYMBOL_GPL(f_hdmi_pcm_get_platform);

static const struct of_device_id hdmi_pcm_dt_ids[] = {
	{ .compatible = "fujitsu,hdmi_pcm" },
	{ /* sentinel */ }
};

MODULE_DEVICE_TABLE(of, hdmi_pcm_dt_ids);

/* PCM driver information of platform device */
static struct platform_driver hdmi_pcm_driver = {
	.driver = {
			.name  = DRIVER_NAME,
			.owner = THIS_MODULE,
			.of_match_table = hdmi_pcm_dt_ids,
	},
	.probe  = hdmi_pcm_probe,
	.remove = hdmi_pcm_remove,
};
module_platform_driver(hdmi_pcm_driver);

MODULE_AUTHOR("Fujitsu Semiconductor Limited");
MODULE_DESCRIPTION("Fujitsu Semiconductor mb8ac0300 PCM DMA module");
MODULE_LICENSE("GPL");
MODULE_ALIAS("*hdmi_pcm*");
