...
static const struct snd_kcontrol_new adau17x1_dsp_controls[] = {
	SOC_SINGLE_EXT("DSP Bypass Playback Switch",
		ADAU17X1_SERIAL_INPUT_ROUTE,
		0, 1, 0, adau17x1_dsp_bypass_get, adau17x1_dsp_bypass_put),
	SOC_SINGLE_EXT("DSP Bypass Capture Switch",
	...
	int source, unsigned int freq_in, unsigned int freq_out)
{
	...
	if (!freq_out) {
		...
		if (freq_out % freq_in != 0) {
			...
			/* Jerry: Fix issue with 48000 base sampling rate */
			r = ((freq_out / freq_in) << 1);
			n = 0;
			m = 0;
			div = 1;
		}
	...
	enum adau17x1_micbias_voltage micbias)
{
	...
	/* hack... jam MICBIAS ON */
	regmap_write(adau->regmap, ADAU17X1_MICBIAS, (micbias << 2) | 1);
