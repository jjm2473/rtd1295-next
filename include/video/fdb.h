/*
 * Fujitsu Display Bus implementation defines
 * Copyright (C) 2013 Linaro, Ltd for Fujitsu Semi
 * Author: Andy Green <andy.green@linaro.org>
 *
 * various things in here based on Rob Clark's omapdrm and dss work
 */

#ifndef __VIDEO_FUJITSU_FDB_H__
#define __VIDEO_FUJITSU_FDB_H__
...
struct f_fdb_ops {
	...
	bool (*get_mirror)(struct f_fdb_child *fdb_child);
	int (*set_mirror)(struct f_fdb_child *fdb_child, bool enable);

	int (*memory_read)(struct f_fdb_child *fdb_child,
			void *buf, size_t size,
			u16 x, u16 y, u16 w, u16 h);

	void (*get_resolution)(struct f_fdb_child *fdb_child,
			u16 *xres, u16 *yres);
	void (*get_dimensions)(struct f_fdb_child *fdb_child,
	...
	int (*set_wss)(struct f_fdb_child *fdb_child, u32 wss);
	u32 (*get_wss)(struct f_fdb_child *fdb_child);
	...
	int (*set_paddr_ref_cb)(
			struct f_fdb_child *fdb_child, dma_addr_t pa, void (*ref_cb)(void *arg), void *arg);
	...
	const struct fdb_format *(*get_fdb_formats)(
	...
	int (*blit_blocking2)(struct f_fdb_child *fdb_child, struct fdb_blit *b,
				struct completion *comp_arg, void *obj_in, void *obj_out);
	int (*get_blit_comp)(struct f_fdb_child *fdb_child,
			     struct b_c_id_ring *b_c_id_ring);
	int (*fdb_debugfs)(struct f_fdb_child *fdb_child,
			   struct seq_file *m, int type);

	int (*fdb_wait_for_vsync)(struct f_fdb_child *fdb_child, int count);
...
struct f_fdb_child {
	...
	struct f_fdb_child *reverse_binding;
};
